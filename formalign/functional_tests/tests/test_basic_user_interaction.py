...
class BasicUserTestCase(StaticLiveServerTestCase):
    ...
    def test_index(self):
        ...
        self.assertEqual(self.live_server_url + '/', r.url, r.url)
        ...
        self.assertEqual('%s/query-sequences' % self.live_server_url, '/'.join(r.url.split('/')[:-2]),
                         '/'.join(r.url.split('/')[:-2]))
    ...
    def test_query_seqs_display(self):
        ...
        self.assertEqual('%s/align-display' % self.live_server_url, '/'.join(r.url.split('/')[:-2]),
                         '/'.join(r.url.split('/')[:-2]))
    ...
    def test_alignment_display(self):
        ...
        r = self.client.get(self.live_server_url + home_button[0].attrib.get('href'))
        self.assertEqual(r.status_code, 200, r.status_code)
